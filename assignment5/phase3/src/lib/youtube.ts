/** YouTube URL → ID and optional metadata (YouTube Data API v3 if `YOUTUBE_API_KEY` is set). */
...
/** Cached metadata fetch (Phase 2 — quota / latency). */
export async function resolveYoutubeMetadataCached(videoId: string): Promise<YoutubeSnippet> {
  const key = `yt:meta:${videoId}`;
  const hit = cacheGet<YoutubeSnippet>(key);
  if (hit) return hit;
  const meta = await resolveYoutubeMetadata(videoId);
  cacheSet(key, meta, 60 * 60 * 1000);
  return meta;
}
