...
import { extractYoutubeVideoId, resolveYoutubeMetadataCached } from "../lib/youtube";
import { schedulePipeline } from "./pipelineService";
...
export async function getVideo(videoId: string) {
  const video = await prisma.video.findUnique({
    ...
    include: {
      ...
      analysis: { include: { recommendedDeck: { select: { id: true, name: true } } } },
    },
  });
    ...
    data: {
      ...
      analysis: video.analysis
        ? {
            ...
            recommendedDeckId: video.analysis.recommendedDeckId,
            recommendedDeck: video.analysis.recommendedDeck,
          }
        : null,
    },
  };
...
export async function createVideo(input: CreateVideoInput) {
  ...
  const meta = await resolveYoutubeMetadataCached(ytId);

  try {
    const video = await prisma.$transaction(async (tx) => {
      const agg = await tx.video.aggregate({
        where: { deckId: input.deckId },
        _max: { queueOrder: true },
      });
      const queueOrder = (agg._max.queueOrder ?? -1) + 1;
